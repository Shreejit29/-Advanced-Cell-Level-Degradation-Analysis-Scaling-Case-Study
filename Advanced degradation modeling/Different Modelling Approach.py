...
for file in files:
    ...
    y_full_lin = lin_model.predict(X)

    X_poly_full = poly.transform(X)
    y_full_poly = poly_model.predict(X_poly_full)

    full_data = y_norm
    X_full_seq, _ = create_seq(full_data, window)
    X_full_seq = X_full_seq.reshape((-1, window, 1))

    y_full_rnn = model_rnn.predict(X_full_seq).flatten()
    y_full_lstm = model_lstm.predict(X_full_seq).flatten()

    cycles_seq = X.flatten()[window:]

   
    plt.figure(figsize=(8,5))

    plt.plot(X.flatten(), y_norm, 'o', label='Actual')
    plt.plot(X.flatten(), y_full_lin, '-', label='Linear')
    plt.plot(X.flatten(), y_full_poly, '--', label='Polynomial')
    plt.plot(cycles_seq, y_full_rnn, '-.', label='RNN')
    plt.plot(cycles_seq, y_full_lstm, ':', label='LSTM')

    plt.xlabel("Cycle Number")
    plt.ylabel("Normalized Capacity")
    plt.title(f"{name} - FULL Model Comparison")
    plt.legend()
    plt.grid()
